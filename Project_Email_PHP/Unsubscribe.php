...
if($row['Email'] == $email){
    if($row['Password'] == $p1)
	{
	$q="UPDATE `subscription` SET Status='no' WHERE Email='$email'";
	mysqli_query($conn,$q);
	echo '<script>alert("Subscription Removed Successfully !")</script>';
	echo '<script> window.location.href="Unsubscribe.php"</script>';
	}else{
...
{
	echo '<script>alert("Login Requried To Remove Subscription !")</script>';
	echo '<script> window.location.href="Unsubscribe.php"</script>';
	}	
}
?>
...
<center><br><br><br><br>
<div id="login"><br>
<h3><b>Unsubscribe Github Timeline</b></h3><p style="color:green">After Unsubscription You Will Not Get Any Update About Github Timeline </p><br>
<form method="post" action="Unsubscribe.php" name="a" onsubmit="return validateform()">
<label>Email</label>
